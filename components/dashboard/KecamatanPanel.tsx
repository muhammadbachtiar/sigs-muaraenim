...
interface KecamatanPanelProps {
  ...
  onAdd: (nama: string, kode: string) => Promise<ActionResult>
  onEdit: (id: string, nama: string, kode: string) => Promise<ActionResult>
  onDelete: (id: string) => Promise<ActionResult>
}

export default function KecamatanPanel({
  title, items, loading, meta, searchValue, onSearchChange, onPageChange, onAdd, onEdit, onDelete,
}: KecamatanPanelProps) {
  const [showAddModal, setShowAddModal] = useState(false)
  const [editItem, setEditItem] = useState<KecamatanItem | null>(null)
  const [deleteItem, setDeleteItem] = useState<KecamatanItem | null>(null)
  ...
  const handleAdd = async () => {
    ...
    const kode = kodeValue.trim() || namaValue.trim().toLowerCase().replace(/\s+/g, '-')
    const result = await onAdd(namaValue.trim(), kode)
    setSubmitting(false)
    if (result.success) {
  ...
  const handleEdit = async () => {
    ...
    setSubmitting(true)
    const kode = kodeValue.trim() || namaValue.trim().toLowerCase().replace(/\s+/g, '-')
    const result = await onEdit(editItem.id, namaValue.trim(), kode)
    setSubmitting(false)
    if (result.success) {
      setEditItem(null)
      setNamaValue('')
    ...
    <div className="space-y-4 animate-in fade-in duration-300">
          ...
          <Table>
            <TableHeader>
              <TableRow className="hover:bg-transparent bg-[var(--color-canvas-soft)]">
                ...
                <TableHead className="w-[180px] text-xs tracking-wider uppercase font-semibold">Kode Kecamatan</TableHead>
                <TableHead className="text-xs tracking-wider uppercase font-semibold">Nama Kecamatan</TableHead>
            ...
            <TableBody>
              {items.map((item, index) => (
                <TableRow key={item.id} className="transition-colors hover:bg-[var(--color-canvas-soft)]/50">
                  ...
                  <TableCell className="font-mono text-xs text-muted-foreground font-medium">{item.kode}</TableCell>
        ...
        <DialogContent className="sm:max-w-[420px]">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="add-nama">Nama Kecamatan</Label>
              <Input
                id="add-nama"
                autoFocus
            ...
            <div className="space-y-1.5">
              <Label htmlFor="add-kode">Kode Kecamatan</Label>
              <Input
                id="add-kode"
                placeholder="slug-kode-kecamatan (opsional)"
                value={kodeValue}
                onChange={(e) => setKodeValue(e.target.value)}
              />
              <span className="text-[11px] text-muted-foreground">Jika dikosongkan, kode akan digenerate otomatis dari nama kecamatan.</span>
            </div>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleAdd} disabled={submitting || !namaValue.trim()}>
      ...
      <Dialog open={!!editItem} onOpenChange={(open) => !open && setEditItem(null)}>
        <DialogContent className="sm:max-w-[420px]">
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              <Label htmlFor="edit-nama">Nama Kecamatan</Label>
              <Input
                id="edit-nama"
                autoFocus
                placeholder="Masukkan nama kecamatan"
            ...
            <div className="space-y-1.5">
              <Label htmlFor="edit-kode">Kode Kecamatan</Label>
              <Input
                id="edit-kode"
                placeholder="slug-kode-kecamatan"
                value={kodeValue}
                onChange={(e) => setKodeValue(e.target.value)}
              />
              <span className="text-[11px] text-muted-foreground">Jika dikosongkan, kode akan digenerate otomatis dari nama kecamatan.</span>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button onClick={handleEdit} disabled={submitting || !namaValue.trim()}>
              {submitting && <div className="w-4 h-4 mr-2 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />}
              Perbarui
            </Button>
          </DialogFooter>
        ...
        <DialogContent className="sm:max-w-[380px]">
          <DialogHeader className="items-center sm:text-center">
            ...
            <DialogDescription className="text-center">
              Apakah Anda yakin ingin menghapus kecamatan <strong>{deleteItem?.nama}</strong>? Tindakan ini tidak dapat dibatalkan.
            </DialogDescription>
          </DialogHeader>
